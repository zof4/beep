...
Beep has three zones. Treat this as the canonical split when adding runtime,
tool, auth, approval, LCM, memory, or proof-command behavior. The detailed
boundary contract lives in
[Agent Runtime Boundaries](./agent-runtime-boundaries.md).

1. **Control plane / authority plane** owns user identity, runtime lifecycle,
   durable auth custody, model gateway policy, capability grants, approvals,
   audit records, external tool dispatch, and typed host brokers.
2. **Runtime house / agent data plane** owns the long-running agent service:
   `beep-agentd`, Pi or Codex harness, request queue, runtime-local event stream,
   sandbox-local tool stubs, LCM service, and runtime state adapters.
3. **Agent workspace / untrusted command area** is where model-directed shell
   commands, file edits, tests, builds, and generated artifacts run. It should be
   useful and powerful, but it must not contain durable refresh tokens,
   operator credentials, approval policy, broker credentials, production control
   plane config, or raw host sockets.

Placement rules:

- LCM belongs in the runtime house as a first-party context engine. The agent
  reaches it through runtime recall tools; the control plane reaches admin
  actions through private runtime endpoints.
- Auth belongs in the control plane/model gateway. Runtime compatibility auth is
  dev-only and must not become production token custody.
- Host-authority tools belong behind the control-plane tool service and typed
  broker. The runtime can expose normal tool stubs to the agent, but it cannot
  execute Docker, domain, secret, deployment, approval, or operator actions
  directly.
- Proof commands should prove these same seams instead of creating special
  bypass paths.
...
flowchart TD
  ...
  ControlPlane --> ToolService["Control-Plane Tool Service"]
  ToolService --> Gatekeeper["Auto-Review Gatekeeper"]
  Gatekeeper --> ToolService
  ToolService --> ToolBroker["Typed Tool Broker"]
...
The control plane owns user identity, runtime lifecycle, capability grants, audit history, external tool dispatch, approval policy, and request flow between the UI and runtime containers.
...
- Receive authority-bearing tool calls from runtimes and pass them through policy, approval, and broker execution.
...
/lcm              runtime-owned Lossless Claw continuity state
/history          runtime-owned workspace history backend storage
/state            runtime-owned state, queues, events, and cursors
/tmp              disposable working directory
```

`/workspace` is the intended model-directed command area. The other mounted
volumes are runtime-service state. If v1 runs all processes under one container
identity, this is still the API ownership model: writes to LCM, history, and
runtime queue state should go through their owning services, not through
ad-hoc workspace commands.
...
- If a local proof uses Codex's `auth.json`, treat it as a temporary dev-only compatibility path, not production architecture. Runtime code must not refresh or own durable ChatGPT tokens.
...
- Register sandbox-local tools plus scoped control-plane-backed tools for authority-bearing capabilities.
...
- Runtime tools that need external capabilities should submit `ToolCallRequest` records to the control-plane tool service.
...
Runtime dependency policy follows the same latest-at-update-lane principle for
the local runtime slice: host wrappers refresh vendored `openai-codex`, `pi`,
and `lossless-claw` on initial use and then after the configured update interval.
The Docker build installs `@openai/codex@latest` and receives
`BEEP_RUNTIME_UPDATE_EPOCH` from the update lane to prevent Docker from keeping
an old cached install layer.

## Module 4: Control-Plane Tool Service

The tool service lives in the control-plane trust boundary. The runtime registers normal harness tools, but tools that need host authority call the control plane with a scoped runtime capability token. This keeps approval policy, durable grants, broker credentials, and cross-user enforcement outside the editable runtime container while still letting the agent call tools directly.

Responsibilities:

- Receive every authority-bearing runtime tool call as a `ToolCallRequest`.
- Authenticate the runtime and bind the call to the owning user, request, and runtime.
...
The tool service is mandatory even for low-risk host-authority tools. This gives us one place for logging, metrics, capability checks, and future policy.
Gatekeeper is not mandatory for every tool. Default-allowed actions execute immediately after authentication and audit; restricted actions return `needs_review` until the gatekeeper evaluates them.
...
flowchart LR
  ...
  subgraph Control["Control Plane Trust Boundary"]
    ToolService["Control-Plane Tool Service"]
...
`beep-agentd` may host the runtime-local request queue, event stream, Pi RPC process, LCM adapter, and sandbox tool stubs. It must not become the durable external tool service or the gatekeeper.
...
sequenceDiagram
  ...
  ToolClient->>ToolService: ToolCallRequest
  ToolService->>ToolService: validate runtime and classify call
  ToolService->>GK: compact context + exact action JSON
  GK->>ToolService: ApprovalDecision
  ToolService->>Audit: append decision
  alt allowed
    ToolService->>Broker: execute typed operation
    Broker->>ToolClient: tool result
    ToolClient->>Agent: tool result
  else denied
    ToolService->>ToolClient: denial + agent-visible reason
...
- Use Traefik if Docker-label traffic mapping is central.
...
- LCM lives in the runtime house as a first-party context engine.
- LCM durable storage lives in runtime-managed volumes and should be mutated
  through `LcmService`/`LcmContextEngine`, not ad-hoc workspace writes.
- LCM recall tools are sandbox-local runtime tools visible to the agent.
- LCM admin actions are control-plane-operated private runtime endpoints.
- LCM is not the file Time Machine.
- LCM is not the semantic memory database.
- LCM is not the preference learner.
- LCM is not the control plane, token vault, approval system, or tool service.

## Module 11: Memory And Preference Learning

Memory should reach the agent through one always-on context assembly path while
remaining split internally into exact continuity, semantic recall, and
structured preferences. Every model call should receive a
bounded, source-backed context floor; provider-specific or memory-source
selection must be handled as ranking and budgeting, not as a hard decision that
some calls bypass.

Layers:

- Lossless Claw for exact continuity.
- Postgres plus pgvector and keyword search for semantic recall.
...
First-party LCM does not require semantic memory. LCM is now the exact
continuity system for the long-running Pi path: always-on passive context
injection, native post-turn ingestion, lifecycle hooks, private admin
maintenance, exact recall tools, model-backed compaction through Beep's
credential adapter, and delegated Pi expansion for `lcm_expand_query`. The
remaining memory work is automated summary-backed hardening and recall
benchmarking, then adding semantic/search memory as the complementary search
and recall layer beside LCM. Semantic/search memory should be another
always-considered context source with separate direct tools and evidence
references back to LCM, not a replacement for LCM. The agent-facing design
lives in [Beep Memory Surface](./beep-memory-surface.md).

## Module 11A: Web Tools

Web access is a control-plane-backed tool surface. The agent sees normal tools
such as `web_search` and `web_fetch`; provider credentials, provider selection,
cost policy, and audit stay in the control plane. Provider integrations are
vendored as Beep-owned HTTP adapters so Tavily, Exa, Brave, Firecrawl, Linkup,
Perplexity Search, and SerpAPI can be swapped or compared without changing the
agent-facing contract.

Design:

- Runtime Pi extension registers `web_search` and `web_fetch`.
- Runtime tool calls go through `/internal/tools/call` with the scoped runtime
  token.
- Control-plane `WebToolService` executes the selected provider adapter.
- Provider outputs normalize to ranked URLs, snippets, optional extracted
  content, provider request IDs, and usage metadata.
- Normal agent calls use `BEEP_WEB_SEARCH_PROVIDER` and
  `BEEP_WEB_FETCH_PROVIDER`, with `BEEP_WEB_TOOL_PROVIDER` available for simple
  single-provider setups. Explicit provider overrides are reserved for
  evaluation.

See [Web Tool Surface](./web-tool-surface.md).
...
sequenceDiagram
  ...
  ToolClient->>ToolService: location.read ToolCallRequest
  ToolService->>Broker: location.read call
...
- Proof commands use the same runtime-house services as production paths and do
  not create legacy/raw-event LCM write paths.
...
### Slice 3: Tool Service And Broker Boundary
...
- Audit logging.
- Tool classification for `sandbox_local`, `runtime_admin_private`,
  `control_plane_default_allowed`, and `control_plane_review_required`.

Acceptance:

- Agent cannot call host tools directly.
- `beep-agentd` cannot bypass the control-plane tool service for external capabilities.
- All authority-bearing tool calls produce ToolCallRequest records.
- One default-allowed control-plane-backed tool succeeds through the tool service.
- One restricted tool returns `needs_review` and cannot be self-approved by the
  runtime token.
...
- Summary-backed LCM hardening.
- Main-agent Pi tools `lcm_grep`, `lcm_describe`, and `lcm_expand_query`, plus
  delegated-only `lcm_expand`.
- Postgres plus pgvector memory store with keyword/BM25-capable retrieval.
...
- Agent can recall relevant prior work through LCM context injection and LCM
  recall tools.
...
- Every authority-bearing tool call has a tool call id.
...
- Traefik for Docker-label traffic mapping.
...
6. Agent calls one low-risk real tool through the runtime tool client, control-plane tool service, and broker.
7. Agent attempts one approval-required restricted tool call.
8. Control-plane tool service returns `needs_review` until the gatekeeper is attached, then the gatekeeper allows, denies, or escalates it.
...
Harden first-party LCM support:

1. Harden always-on passive LCM context injection with automated
   summary-backed tests.
...
5. Keep `lcm_grep`, `lcm_describe`, and `lcm_expand_query` as direct
   main-agent exact-recall tools, with `lcm_expand` restricted to delegated
   expansion sessions.
6. Configure two web providers and run the first web-tool benchmark fixture set.
7. Add semantic/search memory as the complementary search and recall layer
   beside LCM, with both sources considered in context assembly.

The control-plane/runtime boundary proof remains a hardening target, but it
should validate this memory/tool shape rather than a temporary interface.
