...
Beep production rule: durable ChatGPT refresh tokens stay in a control-plane token vault or model gateway, not inside the editable runtime workspace. The runtime gets a narrow model-gateway capability, not raw refresh credentials.
...
Beep equivalent: `Run`, `Turn`, `Item`, `ToolIntent`, `ApprovalRequest`, `ApprovalDecision`, `UsageSnapshot`, and `LcmRecord` events.
...
The gatekeeper is non-conversational, but it is not invisible. It should run from the control-plane trust boundary with the external tool router, not inside `beep-agentd`. The agent receives allow, deny, or escalate results through its runtime tool client and must act accordingly.
