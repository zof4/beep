...
import { executeLcmRecallTool } from "./lcm-recall-tools.mjs";
import { completeLcmModelRequest, resolveLcmModel } from "./lcm-model-completion.mjs";
import { DEFAULT_MODEL } from "./runtime-common.mjs";
...
function buildTurnRuntimeContext({ runtimeContext, tokenBudget, currentTokenCount, usage, sessionStats }) {
  const context = runtimeContext && typeof runtimeContext === "object" && !Array.isArray(runtimeContext)
    ? { ...runtimeContext }
    : {};
  const normalizedCurrentTokenCount = normalizeOptionalPositiveInteger(currentTokenCount);
  if (normalizedCurrentTokenCount !== undefined && context.currentTokenCount === undefined) {
    context.currentTokenCount = normalizedCurrentTokenCount;
  }
  if (usage && typeof usage === "object" && !Array.isArray(usage) && context.usage === undefined) {
    context.usage = cloneJson(usage);
    context.promptCache = {
      ...(context.promptCache && typeof context.promptCache === "object" ? context.promptCache : {}),
      lastCallUsage: cloneJson(usage),
    };
  }
  const normalizedTokenBudget = normalizeOptionalPositiveInteger(tokenBudget);
  if (normalizedTokenBudget !== undefined && context.tokenBudget === undefined) {
    context.tokenBudget = normalizedTokenBudget;
  }
  if (sessionStats && typeof sessionStats === "object" && !Array.isArray(sessionStats)) {
    context.sessionStats = cloneJson(sessionStats);
  }
  return context;
}
...
export class LcmService {
  ...
  async ready() {
    ...
    this.readyPromise = this.initializeReady(modules).finally(() => {
      this.readyPromise = null;
    });
    return this.readyPromise;
  }

  async initializeReady(modules) {
    ...
    const envForConfig = {
      ...
      LCM_FRESH_TAIL_COUNT: process.env.LCM_FRESH_TAIL_COUNT || process.env.BEEP_LCM_FRESH_TAIL_COUNT || "8",
      LCM_SUMMARY_PROVIDER: process.env.LCM_SUMMARY_PROVIDER || "openai-codex",
      LCM_SUMMARY_MODEL: process.env.LCM_SUMMARY_MODEL || process.env.BEEP_PI_CODEX_MODEL || DEFAULT_MODEL,
      LCM_LARGE_FILE_SUMMARY_PROVIDER: process.env.LCM_LARGE_FILE_SUMMARY_PROVIDER || "openai-codex",
      LCM_LARGE_FILE_SUMMARY_MODEL:
        process.env.LCM_LARGE_FILE_SUMMARY_MODEL || process.env.BEEP_PI_CODEX_MODEL || DEFAULT_MODEL,
    };
    ...
    const engine = new modules.LcmContextEngine(
      {
        ...
        complete: completeLcmModelRequest,
        resolveModel: resolveLcmModel,
        callGateway: async () => {
          throw new Error("LCM gateway calls are not available in the Beep runtime service.");
        },
        parseAgentSessionKey: (sessionKey) => {
          if (typeof sessionKey !== "string" || !sessionKey.startsWith("beep:pi:")) return null;
          return { agentId: sessionKey.slice("beep:pi:".length), suffix: "" };
        },
        isSubagentSessionKey: (sessionKey) => String(sessionKey || "").includes(":delegated:"),
        normalizeAgentId: (id = "") => String(id || "").trim(),
        buildSubagentSystemPrompt: () => "",
        readLatestAssistantReply: (messages) => {
  ...
  async ingestPiSession({
    ...
    return this.exclusive(async () => {
      ...
      const shouldRunAfterTurn = selectedEntries.length > 0 || selectedFromMessageCount === 0;
      if (shouldRunAfterTurn) {
        await engine.afterTurn({
          sessionId,
          sessionKey,
          sessionFile: sessionPath,
          messages: allMessages,
          prePromptMessageCount: selectedFromMessageCount,
          tokenBudget: normalizeOptionalPositiveInteger(tokenBudget),
          runtimeContext: buildTurnRuntimeContext({
            runtimeContext,
            tokenBudget,
            currentTokenCount,
            usage,
      ...
      const ingestedMessageDelta = Math.max(0, Number(afterCounts.messages || 0) - Number(beforeCounts.messages || 0));
      return {
        ok: transcript.parseErrors.length === 0,
        mode: "pi-session-after-turn",
        dbPath: this.dbPath,
        ...
        ingest: {
          nativeAfterTurn: {
            ran: shouldRunAfterTurn,
            prePromptMessageCount: selectedFromMessageCount,
            inputMessageCount: allMessages.length,
            selectedMessageCount: selectedEntries.length,
          },
          ingestedMessages: ingestedMessageDelta,
  ...
  async callRecallTool({ sessionId, sessionKey, toolName, params, expansionGrant, delegateExpandQuery }) {
    if (!sessionId) {
      throw new Error("LCM recall tool calls require a sessionId.");
    }
  ...
  async handleBeforeReset({ sessionId, sessionKey, reason = "reset" }) {
    return this.exclusive(async () => {
      const { db, engine } = await this.ready();
      const beforeCounts = rowCounts(db);
      const logStart = this.logs.length;
  ...
  async handleSessionEnd({ sessionId, sessionKey, reason, nextSessionId, nextSessionKey }) {
    return this.exclusive(async () => {
      const { db, engine } = await this.ready();
      const beforeCounts = rowCounts(db);
      const logStart = this.logs.length;
